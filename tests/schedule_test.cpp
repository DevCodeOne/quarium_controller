...
TEST_CASE("Basic schedule testing") {
    ...
    SECTION("Correct schedule testing") {
        ...
        REQUIRE(events.begin()->day() ==
                days(schedule_description["schedule"]["events"][0]["day"].get<unsigned int>()));
    ...
    SECTION("Schedule event sorting") {
        const std::filesystem::path testfiles[] = {"../tests/test_schedules/basic_test_schedule.json"};
