...
bool outputs::add_output(nlohmann::json &gpio_description) {
    ...
    if (id_entry.is_null() || type_entry.is_null() || description_entry.is_null()) {
        logger_instance->critical("A needed entry in a output entry was missing {} {} {}",
                                  id_entry.is_null() ? "id" : "", type_entry.is_null() ? "type" : "",
                                  description_entry.is_null() ? "description" : "");
        if (!id_entry.is_null() && id_entry.is_string()) {
            logger_instance->critical("The issue was in the gpio entry with the id {}", id_entry.get<std::string>());
        }
        return false;
    }

    if (!id_entry.is_string()) {
        logger_instance->critical("The id for a gpio entry is not a string");
        return false;
    }
    ...
    if (is_valid_id(id)) {
        logger_instance->critical("The id {} for a gpio entry is already in use", id);
        return false;
    }

    if (!type_entry.is_string()) {
        logger_instance->critical("The type entry of the output entry {} is not a string", id);
        return false;
    }

    if (!description_entry.is_object()) {
        logger_instance->critical("The description entry for the output entry with the id {} is not an object", id);
        return false;
    }
    ...
    if (!created_output) {
        logger_instance->critical("The output with the id {} couldn't be created", id);
        return false;
    }
